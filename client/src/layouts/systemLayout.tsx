        ...
        <div className="mb-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold text-slate-900" title={currentUser?.full_name || currentUser?.email || "Signed in user"}>
                {currentUser?.full_name || currentUser?.email || "Signed in user"}
              </p>

              <p className="truncate text-xs text-slate-500" title={currentUser?.email || "Secure session"}>
                {currentUser?.email || "Secure session"}
              </p>
            </div>
          </div>

          <span className="mt-3 inline-flex max-w-full items-center rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-blue-700">
